...
public class ExerciseGenerator {
	 ...
	 * @param varCount	maximum number of different variables
	 * @param maxDepth	maximum depth of the generated formulas
	 * @param minLength	minimum length of a (shortest? sort of) refutation 
	 * @param trials	number of attempts to generate
	 * @return a complete ResolutionExercise if it succeeds to generate one in the
	 *   specified number of attempts; <code>null</code> otherwise.
	 */
	public static ResolutionExercise generateExercise(
			int varCount,
			int maxDepth,
			int minLength,
			int trials,
		...
		while( trials-- > 0 ){
			...
			for( Integer clause : sigmaPrime ){
				if( Resolver.isUnit( clause )) units++;
			}
			if( units < minUnits ) continue;
			if( units > maxUnits ) continue;
			Vector<ClauseReason> solution =
				resolver.constructRefutationVector( resolver.refute(sigmaPrime, varCount));
