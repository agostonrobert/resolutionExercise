...
public class Resolver {
	...
	public static class ClauseReason{
		...
		int fromClause1;
		int fromClause2;
		public ClauseReason( int clause ){ this( clause, -1, -1 ); }
	...
	private boolean addClause( int clause ){
		...
		while( minimalIterator.hasNext() ){
			...
			if( isSubsetOf( oldMinimal, clause )) return false;
			if( isSubsetOf( clause, oldMinimal )){
				minimalIterator.remove();
				for( int i = 0; i < bitCount; i++ )	{
					if( (oldMinimal & (1<<i)) != 0 ){ lookupTable.elementAt(i).remove( oldMinimal );}
				}
			}
		}
		minimals.add( clause );
		for( int i = 0; i < bitCount; i++ ){
			if( (clause & (1<<i)) != 0 ){ lookupTable.elementAt(i).add( clause ); }
		}
		return true;
	}
	...
	public Map< Integer, ClauseReason > refute( Set< Integer > sigma, int varCount ){
		...
		for( int i = 0; i < bitCount; i++ ) { lookupTable.add( new HashSet< Integer >() ) ; }
		for( int clause: sigma ){
			if( isTrivial( clause )) continue;
			if( addClause( clause )){
	...
	public Vector< ClauseReason > constructRefutationVector( Map< Integer, ClauseReason > allResolvents ){
	...
	private int constructRefutationVector(
			...
			int clause,
			Map< Integer, Integer > clauseIndexes,
			Vector<ClauseReason> ret) {
		...
		if( reason.fromClause1 != -1 ){
			reason.fromClause1 =constructRefutationVector(allResolvents, reason.fromClause1, clauseIndexes, ret );
			reason.fromClause2 = constructRefutationVector(allResolvents, reason.fromClause2, clauseIndexes, ret );
		}
		ret.add( reason );
