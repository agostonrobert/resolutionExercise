...
public class TruthTableComputer {
	...
	public static Set<Integer> getValuations(Formula formula, boolean value) {
		if (formula instanceof TruthVariable) {
			...
			return Collections.singleton(value ? 1 << (varIndex << 1)
					: 1 << ((varIndex << 1) | 1));
		}
		...
		if ((conn == AssociativeConnective.OR && value)
				|| (conn == AssociativeConnective.AND && !value)) {
		...
		throw new UnsupportedOperationException("Connective " + conn
				+ " is not supported");
	}
}
