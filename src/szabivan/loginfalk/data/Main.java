...
public class Main {
	...
	public static void output(ResolutionExercise exercise, int maxVars,
			int minLength, int trials, PrintStream stream) {
		stream.println("Parameters:\n  # of variables <= " + maxVars
				+ "\n  Length of refutation >= " + minLength + "\n");
		if (exercise == null) {
			stream.println("Could not generate any exercise within " + trials
					+ " trials, giving up. Sorry.");
			return;
		}
	...
	public static void main(String[] args) {
		...
		int maxUnits = getThreshold(minUnits, 0, 2 * maxVars,
				argMap.get("maxUnits"));
		int maxDepth = 3;

		TruthVariable.registerDefaults();

		ResolutionExercise exercise = ExerciseGenerator.generateExercise(
				maxVars, maxDepth, minLength, trials, minUnits, maxUnits);

		output(exercise,
				maxVars,
				minLength,
				trials,
				argMap.containsKey("ascii") ? new PrintStreamWrapper(System.out)
						: argMap.containsKey("latex") ? new PrintStreamLatexWrapper(
