...
public class CompoundFormula implements Formula {
	...
	/**
	 * @param connective		The top-level connective of the compound formula.
	 * @param subformulaArray	The immediate subformulas of the formula, in order.
	 * ENH arity validity is NOT checked for efficiency.
	 * ENH not-null values are NOT checked for efficiency.
	 */
	...
	/**
	 * @return the number of immediate subformulas that are indexed from 0 to {@link #getSubFormulaCount()}-1.
	 */
	public int getSubFormulaCount(){ return subformulas.length; }

	/**
	 * @param index	The index of the requested subformula, should be in 0..{@link #getSubFormulaCount()}-1, inclusive.
	 * @return	The index-th immediate subformula of the formula.
	 * @throws ArrayIndexOutOfBoundsException if the index is outside the bounds.
	 */
	...
	public String toString() {
		...
		if( connective instanceof UnaryConnective ){
			...
			if( !subformulas[0].isAtomic() ) builder.append('(');
			...
			if( !subformulas[0].isAtomic() ) builder.append(')');
			builder.append( connective.toString() );
			if( !subformulas[1].isAtomic() ) builder.append('(');
			...
			for( int i = 0; i < subformulas.length; i++ ){
				if( i > 0 ) builder.append( connective.toString() );
				if( !subformulas[i].isAtomic() ){ builder.append('('); }
				builder.append( subformulas[i].toString() );
				if( !subformulas[i].isAtomic() ){ builder.append(')'); }
			}			
		}else throw new UnsupportedOperationException("How about writing exception texts containing INFORMATION about the problem, eh?");
		...
		if( this.connective instanceof AssociativeConnective ){
			...
			for( Formula subFormula: this.subformulas ){
				if( subFormula instanceof CompoundFormula ){
					...
					}else newSubformulas.add( subFormula );
				}else newSubformulas.add( subFormula );
			}
			return new CompoundFormula( this.connective, newSubformulas.toArray( this.subformulas ) );
		}
		return this;
	}
}
